...
test_reviewed_ralph_runtime_preserves_user_config() {
  ...
  ! grep -Eq '^RALPH_GLOBAL_SKILL_MODEL=' "$config/global-skill.env" ||
    fail_test "reviewed Ralph config forced a global-skill Codex model"
...
test_contract_scoped_ralph_runtime_upgrades_without_overwrite() {
  ...
  (
    ...
    ! grep -Eq '^RALPH_GLOBAL_SKILL_MODEL=' "$first_runtime/config/global-skill.env"
    ...
    ! grep -Eq '^RALPH_GLOBAL_SKILL_MODEL=' "$second_runtime/config/global-skill.env"
