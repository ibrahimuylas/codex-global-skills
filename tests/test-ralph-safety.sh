...
test_runner_blocks_configured_remote() {
  ...
  grep -Fq 'build --skip-push -b codex -n 1' "$TEST_ROOT/args" || fail_test "safe runner did not enforce Ralph skip-push before caller arguments"
...
test_plan_uses_guarded_managed_runtime() {
  ...
  grep -Fq 'plan --skip-push -b codex -n 1 -g Plan the parser --' "$TEST_ROOT/plan-args" || fail_test "guarded plan did not enforce the managed option path"
...
test_runner_resolves_devcontainer_mount() {
  ...
  grep -Fq 'build --skip-push --iterations 1 --backend codex' "$TEST_ROOT/devcontainer-args" || fail_test "guarded runner did not enforce one iteration and the pinned Codex backend"
