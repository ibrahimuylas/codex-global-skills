...
- Run the inner Codex agent with the managed backend Codex home. It reuses the supervising Codex authentication and configuration but exposes no global skills or global `AGENTS.md`, preventing the child from discovering Ralph and recursively starting another loop; project-local instructions still apply.
