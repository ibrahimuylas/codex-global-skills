...
- Use the managed Codex backend and model defaults. Do not pass a backend unless checking or explaining a rejection; the guarded runner sources `global-skill.env`, enforces Codex, and passes the managed model unless the user explicitly requests `--model`.
...
- The guarded runner enforces the reviewed Codex CLI version and managed Codex backend/model defaults on both host and devcontainer. Stop on a missing or mismatched CLI, a missing/modified `global-skill.env`, or a request for another backend.
