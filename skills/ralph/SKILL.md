...
- The guarded runner enforces the reviewed Codex CLI version and Codex backend on both host and devcontainer. Stop on a missing or mismatched CLI or a request for another backend.
...
```bash
<ralph-skill>/scripts/run-plan-guarded.sh -b codex -n 3 -g "Implement the active specs in specs/"
```

If the user names one spec, use:

```bash
<ralph-skill>/scripts/run-plan-guarded.sh -b codex -n 3 -g "Implement <spec path>"
```
