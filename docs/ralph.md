...
<ralph-skill>/scripts/run-plan-guarded.sh -b codex -n 3 -g "Plan the active specs"
<ralph-skill>/scripts/run-build-no-push.sh -b codex -n 1
```
...
Therefore the skill uses guarded plan/build helpers that verify and execute the managed pinned Ralph binary directly (ignoring PATH shadows), normalize every Ralph option, append the pinned Codex backend defensively, substitute byte-for-byte reviewed prompts, and keep initialization independent of Ralph's opinionated scaffold. On the host the default binary lives under `${CODEX_GLOBAL_SKILLS_HOME:-$HOME/.local/share/codex-global-skills}/ralph-runtimes/<runtime-id>/bin/ralph`; inside Ralph's devcontainer it verifies the contract-scoped `/usr/local/bin/ralph` mount.

The runner also enforces the reviewed Codex CLI version and Codex backend. A stale host CLI or a sandbox rebuilt with a different Codex version fails before the agent loop starts.
